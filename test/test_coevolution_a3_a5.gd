...
func _test_hof_update_archives_top_enemies() -> void:
	...
	for i in range(coevo.get_hof_size() - 1):
		assert_gte(coevo.hall_of_fame[i].fitness, coevo.hall_of_fame[i + 1].fitness,
...
func _test_hof_trimmed_to_max_size() -> void:
	...
	assert_eq(coevo.get_hof_size(), CoevolutionScript.HOF_SIZE,
...
func _test_hof_eval_interval() -> void:
	...
	assert_false(coevo.should_eval_against_hof(), "Gen 0 with empty HoF should not eval against HoF")
	...
	assert_eq(coevo.get_generation(), CoevolutionScript.HOF_EVAL_INTERVAL,
		"Should be at gen HOF_EVAL_INTERVAL")
	assert_true(coevo.should_eval_against_hof(),
		"Gen HOF_EVAL_INTERVAL with HoF should eval against HoF")
...
func _test_save_load_hof_roundtrip() -> void:
	...
	assert_approx(coevo2.hall_of_fame[0].fitness, original_fitness, 0.1,
		"Loaded HoF top fitness should match")

...
func _make_coevo(player_pop: int = 20, enemy_pop: int = 20):
	return CoevolutionScript.new(
		player_pop, 86, 32, 6,  # Player: 86 inputs, 32 hidden, 6 outputs
		enemy_pop,              # Enemy population size
		3,                      # Elite count
		0.15,                   # Mutation rate
...
func _test_independent_populations() -> void:
	...
	assert_ne(player_net_0.input_size, enemy_net_0.input_size,
		"Player and enemy networks should have different architectures")


func _test_custom_population_sizes() -> void:
	var coevo = CoevolutionScript.new(
		50, 86, 32, 6,  # 50 players
		30,              # 30 enemies
		5, 0.2, 0.4, 0.8
	)

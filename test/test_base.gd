...
func assert_approx(actual: float, expected: float, epsilon: float = 0.0001, message: String = "") -> void:
	...
	if abs(actual - expected) > epsilon:
		var msg := message if message else "Expected %s to be approximately %s (epsilon=%s)" % [actual, expected, epsilon]
		_fail(msg)
...
func assert_in_range(value: float, min_val: float, max_val: float, message: String = "") -> void:
	...
	if value < min_val or value > max_val:
		var msg := message if message else "Expected %s to be in range [%s, %s]" % [value, min_val, max_val]
		_fail(msg)
...
func assert_array_eq(actual: Array, expected: Array, message: String = "") -> void:
	...
	if actual.size() != expected.size():
		var msg := message if message else "Array size mismatch: %d vs %d" % [actual.size(), expected.size()]
		_fail(msg)
		return

	for i in actual.size():
		if actual[i] != expected[i]:
			var msg := message if message else "Array mismatch at index %d: %s vs %s" % [i, actual[i], expected[i]]
			_fail(msg)
			return

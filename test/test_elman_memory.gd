...
func _run_tests() -> void:
    ...
    # Crossover
    _test(
        "crossover_with_memory_produces_valid_child",
        _test_crossover_with_memory_produces_valid_child
    )
...
func _test_reset_restores_initial_behavior() -> void:
    ...
    for i in out_fresh.size():
        assert_approx(
            out_fresh[i], out_reset[i], 0.0001, "After reset, first forward should match initial"
        )
...
func _test_crossover_with_memory_produces_valid_child() -> void:
    ...
    assert_eq(
        child.get_weight_count(), p1.get_weight_count(), "Child weight count should match parents"
    )

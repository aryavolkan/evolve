...
var bounds: Rect2 = Rect2(
    WALL_MARGIN, WALL_MARGIN, ARENA_SIZE - 2 * WALL_MARGIN, ARENA_SIZE - 2 * WALL_MARGIN
)
...
func _test_obstacle_directions() -> void:
    ...
    var inputs := _compute(
        enemy_pos, 0, Vector2(3000, 3000), Vector2.ZERO, false, false, false, obstacles
    )
...
func _test_no_obstacles_returns_zeros() -> void:
    var inputs := _compute(
        Vector2(1920, 1920), 0, Vector2(2000, 2000), Vector2.ZERO, false, false, false, []
    )
...
func _test_output_size_always_16() -> void:
    ...
    var inputs2 := _compute(
        ...
        true,
        true,
        [Vector2(500, 500), Vector2(1000, 1000), Vector2(2000, 2000)]
    )

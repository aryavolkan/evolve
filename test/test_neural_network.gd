...
func _test_extreme_inputs_stay_bounded() -> void:
	...
	var outputs := nn.forward(inputs)
	for i in outputs.size():
		assert_in_range(outputs[i], -1.0, 1.0, "Large positive inputs should still produce bounded output")

	# Test with very large negative inputs
	inputs.fill(-1000.0)
	outputs = nn.forward(inputs)
	for i in outputs.size():
		assert_in_range(outputs[i], -1.0, 1.0, "Large negative inputs should still produce bounded output")
...
func _test_mutate_with_zero_rate_changes_nothing() -> void:
	...
	var weights_after := nn.get_weights()
	for i in weights_before.size():
		assert_approx(weights_before[i], weights_after[i], 0.0001, "Zero mutation rate should change nothing")

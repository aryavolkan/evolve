...
func _test_network_with_extreme_weights() -> void:
	...
	for i in outputs.size():
		assert_in_range(outputs[i], -1.0, 1.0, "Extreme weights should still produce bounded output")
...
func _test_crossover_identical_parents() -> void:
	...
	for i in weights.size():
		assert_approx(child_weights[i], 0.5, 0.0001, "Crossover with self should produce same weights")

...
func _test_curriculum_stages() -> void:
	...
	assert_gte(CurriculumManagerScript.STAGES.size(), 5.0,
...
func _test_curriculum_config_keys() -> void:
	...
	for stage_config in CurriculumManagerScript.STAGES:
		for key in required_keys:
			assert_true(stage_config.has(key),
				"Stage config has key '%s'" % key)
...
func _test_compare_script_exists() -> void:
	assert_true(FileAccess.file_exists("res://test/integration/compare_reports.py"),
		"compare_reports.py exists")


func _test_gatekeeper_script_exists() -> void:
	assert_true(FileAccess.file_exists("res://test/integration/pr_gatekeeper.sh"),
		"pr_gatekeeper.sh exists")

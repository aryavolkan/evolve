...
func _run_tests() -> void:
	...
	_test("signals_defined", func():
		var screen = GameOverScript.new()
		assert_true(screen.has_signal("restart_requested"), "Should have restart_requested signal")
		assert_true(screen.has_signal("menu_requested"), "Should have menu_requested signal")
	)

	_test("show_stats_sets_visible", func():
		var screen = GameOverScript.new()
		screen.show_stats({
			"score": 5000,
			"kills": 10,
	...
	_test("hide_screen", func():
		var screen = GameOverScript.new()
		screen.show_stats({"score": 100, "kills": 0, "powerups_collected": 0,
			"survival_time": 5.0, "score_from_kills": 0, "score_from_powerups": 0,
			"is_high_score": false, "mode": "play"})
		screen.hide_screen()
		assert_false(screen.visible, "Should be hidden after hide_screen")
	)

	_test("stats_stored", func():
		var screen = GameOverScript.new()
		var data = {"score": 7500, "kills": 20, "powerups_collected": 5,
			"survival_time": 60.0, "score_from_kills": 5000, "score_from_powerups": 2000,
			"is_high_score": true, "mode": "watch"}

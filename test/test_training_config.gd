...
func _test_path_constants_use_user_directory() -> void:
	assert_true(TrainingConfigScript.BEST_NETWORK_PATH.begins_with("user://"), "Best network path should use user://")
	assert_true(TrainingConfigScript.POPULATION_PATH.begins_with("user://"), "Population path should use user://")
	assert_true(TrainingConfigScript.SWEEP_CONFIG_PATH.begins_with("user://"), "Sweep config path should use user://")
	assert_true(TrainingConfigScript.METRICS_PATH.begins_with("user://"), "Metrics path should use user://")
	assert_true(TrainingConfigScript.ENEMY_POPULATION_PATH.begins_with("user://"), "Enemy pop path should use user://")
	assert_true(TrainingConfigScript.ENEMY_HOF_PATH.begins_with("user://"), "Enemy HoF path should use user://")
	assert_true(TrainingConfigScript.MIGRATION_POOL_DIR.begins_with("user://"), "Migration pool dir should use user://")


func _test_path_constants_correct_extensions() -> void:
	assert_true(TrainingConfigScript.BEST_NETWORK_PATH.ends_with(".nn"), "Network file should have .nn extension")
	assert_true(TrainingConfigScript.POPULATION_PATH.ends_with(".evo"), "Population file should have .evo extension")
	assert_true(TrainingConfigScript.SWEEP_CONFIG_PATH.ends_with(".json"), "Sweep config should have .json extension")
	assert_true(TrainingConfigScript.METRICS_PATH.ends_with(".json"), "Metrics file should have .json extension")
...
func _test_get_metrics_path_default() -> void:
	var cfg = TrainingConfigScript.new()
	assert_eq(cfg.get_metrics_path(), "user://metrics.json", "Default metrics path should be user://metrics.json")


func _test_get_metrics_path_with_worker() -> void:
	var cfg = TrainingConfigScript.new()
	cfg.worker_id = "worker_42"
	assert_eq(cfg.get_metrics_path(), "user://metrics_worker_42.json", "Worker metrics path should include worker ID")


func _test_get_raw_missing_key() -> void:
	var cfg = TrainingConfigScript.new()
	assert_null(cfg.get_raw("nonexistent_key"), "Missing key should return null")
	assert_eq(cfg.get_raw("nonexistent_key", 42), 42, "Missing key with default should return default")

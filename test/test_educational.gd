...
func run_tests() -> void:
    ...
    _test(
        "analyze_threats_detects_powerup_when_no_enemy",
        func():
            var overlay = OverlayScript.new()
            var inputs := PackedFloat32Array()
            inputs.resize(86)
    ...
    _test(
        "analyze_threats_returns_none_for_empty",
        func():
            var overlay = OverlayScript.new()
            var inputs := PackedFloat32Array()
            inputs.resize(86)
    ...
    _test(
        "describe_shooting_fires_strongest",
        func():
            var overlay = OverlayScript.new()
    ...
    _test(
        "describe_shooting_no_fire_below_threshold",
        func():
            var overlay = OverlayScript.new()
    ...
    _test(
        "describe_state_shows_invincible",
        func():
            var overlay = OverlayScript.new()
            var inputs := PackedFloat32Array()
            inputs.resize(86)

...
func run_tests() -> void:
    ...
    _test(
        "empty_sensor_inputs_produce_valid_action", _test_empty_sensor_inputs_produce_valid_action
    )
    _test(
        "varied_sensor_inputs_produce_varied_actions",
        _test_varied_sensor_inputs_produce_varied_actions
    )
    _test(
        "network_output_range_compatible_with_controller",
        _test_network_output_range_compatible_with_controller
    )
    ...
    _test(
        "evolved_network_produces_different_behavior",
        _test_evolved_network_produces_different_behavior
    )
...
func _test_network_output_range_compatible_with_controller() -> void:
    ...
    for _trial in 10:
        ...
        var action := get_action_from_outputs(outputs)
        assert_in_range(
            action.move_direction.length(), 0.0, 1.0, "Movement magnitude should be bounded"
        )
...
func _test_pipeline_handles_saturated_inputs() -> void:
    ...
    assert_true(
        (
            action.shoot_direction == Vector2.ZERO
            or action.shoot_direction == Vector2.UP
            or action.shoot_direction == Vector2.DOWN
            or action.shoot_direction == Vector2.LEFT
            or action.shoot_direction == Vector2.RIGHT
        ),

...
func _run_tests() -> void:
	...
	_test("default_config", func():
		var panel = SandboxPanelScript.new()
		var config = panel.get_config()
		assert_true(config.has("enemy_types"), "Config should have enemy_types")
		assert_true(config.has("spawn_rate_multiplier"), "Config should have spawn_rate_multiplier")
	...
	_test("default_spawn_rate", func():
		var panel = SandboxPanelScript.new()
		var config = panel.get_config()
		assert_approx(config.spawn_rate_multiplier, 1.0, 0.01, "Default spawn rate should be 1.0")
	)

	_test("default_network_source", func():
		var panel = SandboxPanelScript.new()
		var config = panel.get_config()
	...
	_test("at_least_one_enemy_type", func():
		var panel = SandboxPanelScript.new()
		# Disable all
	...
	_test("enemy_names_match_types", func():
		var panel = SandboxPanelScript.new()

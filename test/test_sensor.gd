...
func run_tests() -> void:
    ...
    _test(
        "ray_cast_ignores_entity_too_far_from_ray", _test_ray_cast_ignores_entity_too_far_from_ray
    )
...
func cast_ray_to_entities(
    origin: Vector2, direction: Vector2, entities: Array, hit_radius: float
) -> Dictionary:
    ...
    for entity in entities:
        ...
        if dist_to_ray < hit_radius and projection < result.distance:
            ...
            if entity.has_method("get_point_value"):
                ...
                match points:
                    1:
                        result.type_value = 0.2
                    3:
                        result.type_value = 0.5
                    5:
                        result.type_value = 0.8
                    9:
                        result.type_value = 1.0

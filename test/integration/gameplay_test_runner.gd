...
func _next_scenario() -> void:
	...
	match _scenario_name:
		"boot_and_run": _scenario_duration = 300
		"player_movement": _scenario_duration = 120
		"shooting_mechanics": _scenario_duration = 180
		"enemy_spawning": _scenario_duration = 600
...
func _run_scenario_final_check() -> void:
	match _scenario_name:
		"boot_and_run":
			_check(_main_scene != null and is_instance_valid(_main_scene),
		...
		"powerup_collection":
			if _main_scene:
				_check(_main_scene.powerups_collected >= 0,
					"Powerup system functional (collected=%d)" % _main_scene.powerups_collected)

		"player_death_and_respawn":
			if _main_scene:
				_check(_main_scene.lives < 3,
					"Player lost a life (lives=%d)" % _main_scene.lives)
				if _main_scene.lives > 0 and _player:
					_check(not _player.is_hit, "Player respawned")

		"score_tracking":
			if _main_scene:
				_check(_main_scene.score > 0,
					"Score positive (%.0f)" % _main_scene.score)
		...
		"performance_stress":
			if _frame_times.size() > 0:
				...
				print("  Perf: avg=%.1fms max=%.1fms spikes=%.1f%% mem=%.1fMB" % [
					avg, max_ft, spike_pct, _peak_memory_mb])
				_check(spike_pct < 50.0,
					"Frame spikes <50%% (got %.1f%%)" % spike_pct)
				_check(_peak_memory_mb < 2048.0,
					"Memory <2GB (got %.0fMB)" % _peak_memory_mb)
...
func _default_ai_behavior() -> void:
	...
	if nearest_dist < 1500:
		...
		var target := arena_center + Vector2(
			sin(_frame_count * 0.02) * 500,
			cos(_frame_count * 0.03) * 500)

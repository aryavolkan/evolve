...
func _make_outputs_with_peak(
    peak_idx: int, peak_val: float = 0.9, base_val: float = -0.5
) -> PackedFloat32Array:
...
func _test_knight_returns_l_shaped_move() -> void:
    ...
    var is_l_shape := (
        (is_equal_approx(ax, 2.0) and is_equal_approx(ay, 1.0))
        or (is_equal_approx(ax, 1.0) and is_equal_approx(ay, 2.0))
    )
...
func _test_knight_move_is_tile_scaled() -> void:
    ...
    for dir_idx in NUM_OUTPUTS:
        ...
        var ax := absf(move.x)
        var ay := absf(move.y)
        var is_tile_multiple_x := (
            is_equal_approx(fmod(ax, TILE_SIZE), 0.0) or is_equal_approx(ax, 0.0)
        )
        var is_tile_multiple_y := (
            is_equal_approx(fmod(ay, TILE_SIZE), 0.0) or is_equal_approx(ay, 0.0)
        )
...
func _test_bishop_picks_diagonal_only() -> void:
    ...
    for dir_idx in [DIR_NE, DIR_SE, DIR_SW, DIR_NW]:
        ...
        assert_approx(
            absf(move.x), absf(move.y), 0.01, "Bishop move should be diagonal (|x| == |y|)"
        )

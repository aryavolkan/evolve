...
func _test_enemy_type_filtering() -> void:
	...
	for spawn in events2.enemy_spawns:
		assert_true(spawn.type == 0 or spawn.type == 1,
			"Enemy type %d should be 0 or 1" % spawn.type)
...
func _test_powerup_type_filtering() -> void:
	...
	for spawn in events.powerup_spawns:
		assert_true(spawn.type == 0 or spawn.type == 6,
			"Powerup type %d should be 0 or 6" % spawn.type)
...
func _test_obstacle_scaling() -> void:
	...
	assert_lt(events_small.obstacles.size(), events_full.obstacles.size(),
		"Small arena should have fewer obstacles")
	assert_gte(events_small.obstacles.size(), 3.0,
		"Small arena should have at least 3 obstacles")
...
func _test_stage0_nursery() -> void:
	...
	for spawn in events.powerup_spawns:
		assert_true(spawn.type == 0 or spawn.type == 6,
			"Nursery powerup type %d not in [0, 6]" % spawn.type)


func _test_stage4_final() -> void:
	# Final stage should allow all types
	var config = {"arena_scale": 1.0, "enemy_types": [0, 1, 2, 3, 4],

...
## Current State
- Godot user data: !`echo ~/.local/share/godot/app_userdata/Evolve`

## Instructions

Run a single Python script that collects all worker information and prints a consolidated report. Use the following bash command:
...
# ── 1. Python worker processes (filter to actual python processes, not bash wrappers) ──
...
for l in ps.stdout.splitlines():
    if ('overnight_evolve.py' in l or 'overnight_sweep.py' in l) and 'grep' not in l:
        parts = l.split()
        cmd = ' '.join(parts[10:])
        # Skip bash wrapper processes — only count actual python processes
...
# ── 3. Metrics files (current training progress) ──
print()
...
# ── 4. Worker log files (check multiple locations) ──
print()
...
if py_workers and not godot_procs:
    warnings.append('Python workers running but no Godot instances — workers may be idle (sweep paused?) or between runs')
...
if godot_procs and not active_metrics:
    warnings.append('Godot running but no recent metrics — training may not have started yet or metrics path mismatch')
if warnings:
    print()
    print('=== Warnings ===')
    for w in warnings:

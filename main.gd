...
const GameStateManagerScript = preload("res://game_state_manager.gd")
const DifficultyScalerScript = preload("res://difficulty_scaler.gd")
const InputCoordinatorScript = preload("res://input_coordinator.gd")
...
func _apply_sandbox_starting_state() -> void:
    if not sandbox_overrides_active:
        return
    score = sandbox_starting_difficulty * DifficultyScalerScript.DIFFICULTY_SCALE_SCORE
...
static var EventGenerator = load("res://scripts/event_generator.gd")
...
func _ready() -> void:
    ...
    game_state = GameStateManagerScript.new()
    difficulty = DifficultyScalerScript.new()
    ...
    var proj_pool = ObjectPool.new(preload("res://projectile.tscn"), self)
    ...
    input_coord = InputCoordinatorScript.new()
                ...
                if pop:
                    ...
                    for i in pop.pop_size:
                        if pop.fitnesses[i] > best_fit:
                            ...
                            if pop.lineage_ids.size() > i:
                                best_id = pop.lineage_ids[i]
            if best_id >= 0:
...
func spawn_initial_enemies() -> void:
    spawn_mgr.spawn_initial_enemies(training_mode, DifficultyScalerScript.BASE_ENEMY_SPEED, enemy_ai_network)
...
func _on_player_hit() -> void:
    ...
    if game_over:
        ...
        var arena_center = Vector2(effective_arena_width / 2, effective_arena_height / 2)
        player.respawn(arena_center, GameStateManagerScript.RESPAWN_INVINCIBILITY)
...
var ArenaSetup = preload("res://arena_setup.gd")

func setup_arena() -> void:
    var arena_setup_mgr = ArenaSetup.new()
...
func update_camera_zoom() -> void:
    ArenaSetup.update_camera_zoom(arena_camera, effective_arena_width, effective_arena_height)
...
func get_arena_bounds() -> Rect2:
    return ArenaSetup.get_arena_bounds(effective_arena_width, effective_arena_height)

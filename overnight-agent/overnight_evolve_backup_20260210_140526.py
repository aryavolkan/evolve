    ...
    while time.time() - start_time < timeout_minutes * 60:
        ...
        try:
            ...
            if gen > last_gen:
                ...
                # Conditional metrics (only log if present to avoid noise)
                if 'pareto_front_size' in data:
                    log_data['pareto_front_size'] = data['pareto_front_size']
                    log_data['hypervolume'] = data.get('hypervolume', 0)
                if 'neat_species_count' in data:
                    log_data['neat_species_count'] = data['neat_species_count']
                    log_data['neat_compatibility_threshold'] = data.get('neat_compatibility_threshold', 0)
